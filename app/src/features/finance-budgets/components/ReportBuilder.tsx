...
import { REPORT_ROWS } from '../data/financeMock'
...
export function ReportBuilder() {
  ...
  const net = REPORT_ROWS.reduce((sum, row) => sum + (row.spent - row.allocated), 0)

  return (
    <div className="mt-4 flex flex-col gap-4">
      <div>
        <p className="text-xs font-semibold uppercase tracking-[0.06em] text-text-3">Report</p>
        <h2 className="mt-1 text-lg font-semibold text-text">Budgeting — Report</h2>
      ...
      <div className="rounded-lg border border-line bg-paper p-5">
        <div className="flex items-center justify-between">
          <span className="text-xs text-text-3">{REPORT_ROWS.length} rows</span>
          <span className={`text-sm font-semibold ${net <= 0 ? 'text-bad' : 'text-ok'}`}>
            Net variance: {net <= 0 ? '-' : '+'}KWD {Math.abs(net).toLocaleString()}.000
          </span>
        </div>
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="border-b border-line">
                {['Budget #', 'Line', 'Department', 'Type', 'Allocated (KWD)', 'Spent (KWD)', 'Variance'].map((label) => (
            ...
            <tbody>
              {REPORT_ROWS.map((row) => {
                const variance = row.spent - row.allocated
                return (
                  <tr key={row.budgetNo} className="border-b border-line last:border-b-0">
                    <td className="px-3 py-2 font-medium text-text">{row.budgetNo}</td>
                    <td className="px-3 py-2 text-text-2">{row.line}</td>
                    <td className="px-3 py-2 text-text-2">{row.department}</td>
                    <td className="px-3 py-2 text-text-2">{row.type}</td>
                    <td className="px-3 py-2 text-text-2">{row.allocated.toLocaleString()}.000</td>
                    <td className="px-3 py-2 text-text-2">{row.spent.toLocaleString()}.000</td>
                    <td className={`px-3 py-2 font-medium ${variance <= 0 ? 'text-bad' : 'text-ok'}`}>
                      {variance <= 0 ? '-' : '+'}
                      {Math.abs(variance).toLocaleString()}.000
                    </td>
                  </tr>
                )
              })}
