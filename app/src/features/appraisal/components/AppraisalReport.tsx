...
import { APPRAISAL_REPORT_ROWS } from '../data/appraisalMock'
...
export function AppraisalReport() {
  ...
  const avgScore = APPRAISAL_REPORT_ROWS.reduce((sum, row) => sum + row.score, 0) / APPRAISAL_REPORT_ROWS.length

  return (
    <div className="mt-4 flex flex-col gap-4">
      <div>
        <p className="text-xs font-semibold uppercase tracking-[0.06em] text-text-3">Report</p>
        <h2 className="mt-1 text-lg font-semibold text-text">Appraisal — Report</h2>
      ...
      <div className="rounded-lg border border-line bg-paper p-5">
        <div className="mb-3 flex items-center justify-between">
          <span className="text-xs text-text-3">{APPRAISAL_REPORT_ROWS.length} rows</span>
          <span className="text-sm font-semibold text-text">Average score: {avgScore.toFixed(2)} / 5.00</span>
        </div>
        <AppraisalTable rows={APPRAISAL_REPORT_ROWS} />
      </div>
