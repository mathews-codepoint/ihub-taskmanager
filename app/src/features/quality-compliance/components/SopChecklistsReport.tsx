...
import { SOP_REPORT_ROWS } from '../data/qcMock'

const FILTER_FIELDS = [
  { key: 'subject', label: 'Action sheet number / subject', type: 'text' as const, placeholder: 'AS-2026-114 or type a subject' },
...
/** Matches a known prototype quirk: the Report toggle on both Dashboard
 * and Observations loads this identical "SOP Checklists — Report" view
 * with the same sample rows rather than a per-tab report — carried over
 * faithfully per Standing Decision #1. */
export function SopChecklistsReport() {
  const [filtersOpen, setFiltersOpen] = useState(false)
  const [filters, setFilters] = useState<FilterValues>({})
  const [exportOpen, setExportOpen] = useState(false)

  const avgCompletion = SOP_REPORT_ROWS.reduce((sum, row) => sum + row.completionPct, 0) / SOP_REPORT_ROWS.length

  return (
    <div className="mt-4 flex flex-col gap-4">
      <div>
        <p className="text-xs font-semibold uppercase tracking-[0.06em] text-text-3">Report</p>
        <h2 className="mt-1 text-lg font-semibold text-text">SOP Checklists — Report</h2>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <label className="flex h-10 flex-1 min-w-[220px] items-center gap-2 rounded-sm border border-line-2 bg-bg px-3">
          ...
          <input placeholder="AS-2026-114 or type a subject" className="h-full flex-1 bg-transparent text-sm text-text outline-none" />
        </label>
        <button
          type="button"
      ...
      <div className="rounded-lg border border-line bg-paper p-5">
        <div className="flex items-center justify-between">
          <span className="text-xs text-text-3">{SOP_REPORT_ROWS.length} rows</span>
          <span className="text-sm font-semibold text-text">Average completion: {avgCompletion.toFixed(1)}%</span>
        </div>
        <div className="mt-3 overflow-x-auto">
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="border-b border-line">
                {['Action sheet #', 'Checklist', 'Zone', 'Completed by', 'Date', 'Completion'].map((label) => (
            ...
            <tbody>
              {SOP_REPORT_ROWS.map((row) => (
                <tr key={row.id} className="border-b border-line last:border-b-0">
                  <td className="px-3 py-2 font-medium text-text">{row.id}</td>
                  <td className="px-3 py-2 text-text-2">{row.checklist}</td>
                  <td className="px-3 py-2 text-text-2">{row.zone}</td>
                  <td className="px-3 py-2 text-text-2">{row.completedBy}</td>
                  <td className="px-3 py-2 text-text-2">{row.date}</td>
                  <td className="px-3 py-2 text-text-2">{row.completionPct}%</td>
                </tr>
              ))}
            </tbody>
          </table>
